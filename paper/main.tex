...
\begin{figure}[h]
  ...
  \caption{\geore{} architecture. RGB, language and proprioception enter the PaliGemma encoder; pre-fusion SigLIP image tokens fork into two parallel transformer experts. The \emph{action} branch is the open-$\pi_0$ action expert plus a flow-matching head emitting the next 4-step action chunk. The \emph{depth} branch is a same-shape transformer that produces $16\!\times\!16$ logits over a frozen $K{=}256$ VQ-VAE depth codebook; cross-entropy against simulator-GT depth tokens (lower row) yields the auxiliary $\Ldepth$ term. The codebook is pretrained then frozen; the supervision path is active only at training time.}
  \label{fig:arch}
\end{figure}
