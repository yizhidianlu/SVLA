...
\begin{table}[h]
  \centering
  \caption{Recipe-isolation ablation on LIBERO-Spatial. v1 $\to$ v2 isolates the training-recipe contribution; v2 $\to$ Stage-C isolates the warmstart + multi-suite pretrain contribution; Stage-C vs.\ QDepth-VLA isolates the depth-CE auxiliary head, the only architectural delta between the two systems.}
  \label{tab:ablation}
  \begin{tabular}{lcccc}
    \toprule
    Run & Warmstart & LIBERO-90 pretrain & Finetune (eff.\ batch) & SR (\%) \\
    \midrule
    v1                    & \xmark & \xmark & 5 ep / 16   & XX.X \\
    v2                    & \xmark & \xmark & 6.5 ep / 64 & XX.X \\
    Stage-C (ours)        & \cmark & 20 ep  & 50 ep / 1024 & \textbf{XX.X} \\
    \midrule
    QDepth-VLA~\citep{qdepthvla2025} & \cmark & 20 ep & 50 ep / 1024 & 86.0 \\
    \bottomrule
  \end{tabular}
\end{table}
